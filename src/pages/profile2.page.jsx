...
const ProfilePage3 = () => {
    ...
    const fetchFollowData = async () => {
        try {
            const followersResponse = await axios.get(`${import.meta.env.VITE_SERVER_DOMAIN}/user/${profileId}/followers`);
            const followingResponse = await axios.get(`${import.meta.env.VITE_SERVER_DOMAIN}/user/${profileId}/following`);
            setFollowerCount(followersResponse.data.length);
            setFollowingCount(followingResponse.data.length);
            setIsFollowing(followersResponse.data.some(follower => follower._id === username));
    ...
    const handleFollow = async () => {
        try {
            await axios.post(`${import.meta.env.VITE_SERVER_DOMAIN}/user/${profileId}/follow`, { followerId: username });
            setIsFollowing(true);
    ...
    const handleUnfollow = async () => {
        try {
            await axios.post(`${import.meta.env.VITE_SERVER_DOMAIN}/user/${profileId}/unfollow`, { followerId: username });
                    ...
                    profile_username.length ?
                        <section className="h-cover md:flex flex-row-reverse items-start gap-5 min-[1100px]:gap-12">
                            ...
                            <div className="max-md:mt-12 w-full">
                                <InPageNavigation
                                    routes={[`Blogs Published`, `Followers (${followerCount})`, `Following (${followingCount})`, `About`]}
                                ...
                                >
                                    ...
                                    <div>
                                        {followers.map((follower, index) => (
                                            <UserCard2 key={index} user={follower} />
                                        ))} : <NoDataMessage message="No Follower" />
                                    </div>
                                    </>
                                    <>
                                    <div>
                                        {following.map((followedUser, index) => (
                                            <UserCard2 key={index} user={followedUser} />
